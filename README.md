...
C++20과 Qt 6.8.3 Core를 사용하는 버전 0.1.0의 동적 라이브러리이다. iisacc.com 계정 모델을 확장한 `FileAuthor`가 파일 작성자의 신원·상세 프로필·기여 정보·작성 디바이스를 기록하고, `AuthenticationToken`이 별도의 런타임 인증 토큰을 보유한다. 파일 읽기·쓰기, 로그인 HTTP 요청과 JWT 서명 검증은 이 객체의 역할에 포함되지 않는다.
...
`<iiFileProvider.h>` 하나로 작성자 모델과 인증 토큰을 사용할 수 있다. 각각의 공개 헤더는 `FileAuthor.h`, `AuthenticationToken.h`이다. 클래스는 QObject가 아닌 C++ 값 객체이며 Qt Network나 계정 매니저의 수명에 의존하지 않는다.
